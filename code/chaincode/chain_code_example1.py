...
# Global var for region_grow DIRECTIONs
DIRECTION =  []
...
def region_grow (i,j,g_min,g_max,dir):
    """
    finds regions in the image and saves DIRECTION
    """
    was_not_there = labelled[i,j]==0
    within_pixel_boundaries = array_img[i,j]>g_min and array_img[i,j]<g_max
    should_perform = within_pixel_boundaries and was_not_there
    if should_perform:
        DIRECTION.append(dir)
        # Mark as found
        labelled[i,j]=1
        within_image_boundaries = j < width_img-1 and j > 1 and i < hight_img-1 and i > 1
        if within_image_boundaries :
            # DIRECTIONs = [
            #     {'i': -1, 'j':0,'dir':1}
            # ]
            # for dir in DIRECTIONs:
            #     region_grow(dir['i'], dir['j'], g_min, g_max,dir['dir'])
